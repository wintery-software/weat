...
import { type HTMLAttributes, createContext, useContext, useEffect } from "react";
...
export const RelativeTimeZoneDisplay = ({ className, ...props }: RelativeTimeZoneDisplayProps) => {
  ...
  return (
    <div className={cn("pl-8 tabular-nums text-muted-foreground", className)} {...props}>
      {display}
    </div>
  );
};
...
export const RelativeTimeZoneLabel = ({ className, ...props }: RelativeTimeZoneLabelProps) => (
  <div
    className={cn("rounded-xs flex h-4 items-center justify-center bg-secondary px-1.5 font-mono", className)}
    {...props}
  />
);
